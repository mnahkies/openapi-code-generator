...
export class ApiClient extends AbstractFetchClient {
  ...
  async getAccount(
    ...
    const res = await fetch(url + query, { method: "GET", headers, body })
  ...
  async postTaxCalculations(p: {
    ...
    Response<200, t_tax_calculation> | Response<StatusCode, t_error>
  > {
  ...
  async postTerminalReaders(p: {
    ...
    Response<200, t_terminal_reader> | Response<StatusCode, t_error>
  > {
    const url = this.basePath + `/v1/terminal/readers`
    const headers = this._headers({
      "Content-Type": "application/x-www-form-urlencoded",
