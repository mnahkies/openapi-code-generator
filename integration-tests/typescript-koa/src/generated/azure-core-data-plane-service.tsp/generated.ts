...
import {
  ...
  t_WidgetsGetWidgetOperationStatusParamSchema,
  t_WidgetsGetWidgetOperationStatusQuerySchema,
...
export type WidgetsGetWidgetOperationStatusResponder = {
  with200(): KoaRuntimeResponse<{
...
export type WidgetsGetWidgetOperationStatus = (
  params: Params<
    t_WidgetsGetWidgetOperationStatusParamSchema,
    t_WidgetsGetWidgetOperationStatusQuerySchema,
    void
  >,
  respond: WidgetsGetWidgetOperationStatusResponder,
  ctx: RouterContext,
) => Promise<
  | KoaRuntimeResponse<unknown>
  | Response<
...
export type Implementation = {
  ...
  widgetsGetWidgetOperationStatus: WidgetsGetWidgetOperationStatus
...
export function createRouter(implementation: Implementation): KoaRouter {
  ...
  const widgetsGetWidgetOperationStatusParamSchema = z.object({
    widgetName: z.string(),
    operationId: z.string(),
  })

  const widgetsGetWidgetOperationStatusQuerySchema = z.object({
    "api-version": z.string().min(1),
  })

  const widgetsGetWidgetOperationStatusResponseValidator =
    responseValidationFactory(
      [
        [
          "200",
    ...
    async (ctx, next) => {
      const input = {
        params: parseRequestInput(
          widgetsGetWidgetOperationStatusParamSchema,
          ctx.params,
          RequestInputType.RouteParam,
        ),
        query: parseRequestInput(
          widgetsGetWidgetOperationStatusQuerySchema,
          ctx.query,
          RequestInputType.QueryString,
        ),
        body: undefined,
      ...
      const response = await implementation
        .widgetsGetWidgetOperationStatus(input, responder, ctx)
        .catch((err) => {
          throw KoaRuntimeError.HandlerError(err)
        })
      ...
      ctx.body = widgetsGetWidgetOperationStatusResponseValidator(status, body)
      ctx.status = status
      return next()
    },
  )
