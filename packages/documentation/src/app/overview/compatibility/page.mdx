...
| enum                 |     ✅     | Applies to `type: number`, `type: string` and `type: boolean`. See also [enums guide](../guides/concepts/enums)            |
| type                 |     ✅     |                                                                                                                            |
...
| format               |   ✅/🚧    | Support for format `binary`, `email`, `date-time`, `date`, `time`                                                          |
| default              |     ✅     |                                                                                                                            |
| nullable             |     ✅     | Also supports `type: null` as an alternative                                                                               |
| discriminator        |   ✅/🚧    | Well supported when union is using $ref. Ignored for in-line schemas. `defaultMapping` not yet supported.                  |
