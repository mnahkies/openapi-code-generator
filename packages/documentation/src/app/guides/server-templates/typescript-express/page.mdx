...
npm i --save @nahkies/typescript-express-runtime express cors body-parser zod
```
...
### Middleware
Each `createRouter` function accepts an optional `options: {middleware: RequestHandler[]}` parameter. You can use this
to add middleware to the router.

```typescript
import {createRouter} from "../generated"

const router = createRouter({/*route implementations*/}, {
  middleware: [
    // example logging middleware
    (req, res, next) => {
      console.log(`Request started: ${req.method} ${req.path}`)
      res.on("finish", () => {
        console.log(`Request completed: [${res.statusCode}] ${req.method} ${req.path}`)
      })
      next()
    }
  ]
